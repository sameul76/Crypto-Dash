...
def calculate_pnl_and_metrics(trades_df: pd.DataFrame):
    ...
    df = trades_df.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce", utc=False)  # <-- naive, as stored
    ...
    for asset, data in positions.items():
        if data["quantity"] > Decimal("1e-9"):
            ...
            if not latest_market.empty:
                latest_row = latest_market.sort_values('timestamp').iloc[-1]
...
def download_drive_file_bytes(url_or_id: str) -> bytes | None:
    ...
    try:
        ...
        with requests.Session() as s:
            r1 = s.get(base, params={"id": fid, "export": "download"}, stream=True, timeout=60)
            if "text/html" in (r1.headers.get("Content-Type") or ""):
                m = re.search(r"confirm=([0-9A-Za-z_-]+)", r1.text)
                if m:
                    params = {"id": fid, "confirm": m.group(1), "export": "download"}
...
def _read_parquet_bytes(b: bytes, label: str) -> pd.DataFrame:
    ...
    if b[:4] != b"PAR1":
        try:
            ...
            st.warning(f"{label}: bytes not Parquet; parsed as CSV fallback.")
            return df_csv
        except Exception:
...
def load_data(trades_link: str, market_link: str):
    ...
    if not trades.empty:
        ...
        # IMPORTANT: read timestamps exactly as stored (naive)
        if "timestamp" in trades.columns:
            trades["timestamp"] = pd.to_datetime(trades["timestamp"], errors="coerce", utc=False)
    ...
    if not market.empty:
        ...
        if "timestamp" in market.columns:
            market["timestamp"] = pd.to_datetime(market["timestamp"], errors="coerce", utc=False)
        ...
        # numeric columns as float (for plotting)
...
def check_auto_refresh():
    now = time.time()
    elapsed = now - st.session_state.get("last_refresh", 0)
...
elapsed_since_refresh = check_auto_refresh()
...
# Debug panel – shows raw, unchanged timestamps
with st.expander("🔍 Data Freshness Debug", expanded=True):
    if not trades_df.empty and 'timestamp' in trades_df.columns:
        st.write("**Latest Trade Timestamp (raw):**", str(trades_df['timestamp'].max()))
        st.write("**Total Trades:**", len(trades_df))
    if not market_df.empty and 'timestamp' in market_df.columns:
        st.write("**Latest Market Timestamp (raw):**", str(market_df['timestamp'].max()))
        st.write("**Total Market Records:**", len(market_df))
    st.write(f"**Cache age:** {int(time.time() - st.session_state.last_refresh)}s")
...
with st.sidebar:
    ...
    with col1:
        st.session_state.auto_refresh_enabled = st.toggle("🔄 Auto-Refresh (5min)", value=st.session_state.get("auto_refresh_enabled", True))
    with col2:
    ...
    if st.session_state.get("auto_refresh_enabled", True):
        time_until = max(0, REFRESH_INTERVAL - int(elapsed_since_refresh))
        minutes, seconds = divmod(time_until, 60)
        st.markdown(
            f"<p style='text-align:center;font-size:.9em;color:#4CAF50;'>⏱️ Next refresh: {minutes:02d}:{seconds:02d}</p>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown("<p style='text-align:center;font-size:.9em;color:#888;'>Auto-refresh disabled</p>", unsafe_allow_html=True)

    st.markdown("---")

    # Date range source (use raw naive timestamps)
    date_source = "No Data"
    ...
    if min_date is not None and max_date is not None:
        ...
        st.markdown(f"<p style='text-align:center;font-size:.8em;color:grey;'>Source: {date_source}</p>", unsafe_allow_html=True)
    else:
        st.markdown("<p style='text-align:center;color:orange;'>⚠️ No Date Range Available</p>", unsafe_allow_html=True)

    # Latest data time (raw)
    ...
    if latest_data_time:
        st.markdown(
            f"<p style='text-align:center;font-size:.9em;color:grey;'>{data_source_for_time}: {latest_data_time.strftime('%Y-%m-%d %H:%M')}</p>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown("<p style='text-align:center;font-size:.9em;color:grey;'>No data timestamps available</p>", unsafe_allow_html=True)
    ...
    if pnl_summary:
        total_pnl = float(sum(pnl for pnl in pnl_summary.values() if pd.notna(pnl)))
...
with tab1:
    ...
    if assets:
        ...
        with st.expander("🔍 Data Resolution Inspector"):
            if not asset_market_data.empty:
                ...
                with col1:
                    ...
                    st.write(f"**Date Range:** {min_ts} → {max_ts}")
                with col2:
                ...
                with col3:
                    ...
                    st.write("**Last 10 Timestamps (raw):**")
        ...
        if not df.empty:
            ...
            if not vis.empty:
                st.info(f"Showing {len(vis):,} candles from {start_date} to {end_date}")
                ...
                if 'p_up' in vis.columns and 'p_down' in vis.columns:
                    ...
                    if not prob_data.empty:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            hovertemplate='<b>ML Signal</b><br>Time: %{x|%Y-%m-%d %H:%M}<br>Price: $%{y:.6f}'
                                          '<br>P(Up): %{customdata[0]:.3f}<br>P(Down): %{customdata[1]:.3f}'
                ...
                if not trades_df.empty:
                    asset_trades = trades_df[(trades_df["asset"] == selected_asset) &
                                             ...
                                             (trades_df["timestamp"] <= end_date)].copy()
                    if not asset_trades.empty:
                        ...
                        if not buy_trades.empty:
                            ...
                            fig.add_trace(go.Scatter(
                                x=buy_trades["timestamp"], y=[marker_y_position] * len(buy_trades),
                        ...
                        if not sell_trades.empty:
                            ...
                            fig.add_trace(go.Scatter(
                                x=sell_trades["timestamp"], y=[marker_y_position] * len(sell_trades),
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(title="Price (USD)",
                               ...
                               showgrid=True, gridcolor='rgba(128,128,128,0.1)', range=[y_min_range, y_max_range]),
                    hovermode="x unified",
                ...
                asset_trades = trades_df[(trades_df["asset"] == selected_asset) &
                                         (trades_df["timestamp"] >= start_date) &
                                         (trades_df["timestamp"] <= end_date)].copy() if not trades_df.empty else pd.DataFrame()
                if not asset_trades.empty:
                ...
                if 'p_up' in vis.columns and 'p_down' in vis.columns:
                    ...
                    if not prob_data.empty:
                        ...
                        with col1:
                            bullish = len(prob_data[prob_data['p_up'] > prob_data['p_down']])
                            st.metric("Bullish Signals", f"{bullish} ({bullish/len(prob_data)*100:.1f}%)")
                        with col2:
                            bearish = len(prob_data[prob_data['p_down'] > prob_data['p_up']])
                            st.metric("Bearish Signals", f"{bearish} ({bearish/len(prob_data)*100:.1f}%)")
                        with col3:
                            avg_conf = abs(prob_data['p_up'] - prob_data['p_down']).mean()
                            st.metric("Avg Confidence", f"{avg_conf:.3f}")
                        with col4:
                            high_conf = len(prob_data[abs(prob_data['p_up'] - prob_data['p_down']) > 0.1])
...
with tab2:
    if not trades_df.empty and "timestamp" in trades_df.columns and "cumulative_pnl" in trades_df.columns:
        ...
        fig_pnl = go.Figure()
        fig_pnl.add_trace(go.Scatter(x=plot_df["timestamp"], y=plot_df["cumulative_pnl"], mode="lines",
        ...
        fig_pnl.update_layout(title="Total Portfolio P&L", template="plotly_white",

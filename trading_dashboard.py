...
# High precision for money math we do locally
getcontext().prec = 30
...
def normalize_prob_columns(df: pd.DataFrame) -> pd.DataFrame:
    # Keep only p_up / p_down (no p_hold in this app view)
    rename_map = {}
    ...
    for col in df.columns:
        ...
        if col_key in p_up_variations:
            rename_map[col] = "p_up"
        elif col_key in p_down_variations:
...
def calculate_pnl_and_metrics(trades_df: pd.DataFrame):
    ...
    gp, gl, peak, mdd = Decimal(0), Decimal(0), Decimal(0), Decimal(0)
    ...
    for asset, data in positions.items():
        if data["quantity"] > Decimal("1e-9"):
            ...
            if not latest_market.empty:
                latest_price = Decimal(str(latest_market.sort_values("timestamp").iloc[-1]["close"]))
...
def load_data(trades_link: str, market_link: str):
    ...
    if not trades.empty:
        ...
        if "value" in trades.columns:
            column_mapping["value"] = "usd_value"
        ...
        if "action" in trades.columns:
            trades["unified_action"] = (
                trades["action"].str.upper().map({"OPEN": "buy", "CLOSE": "sell"}).fillna(trades["action"].str.lower())
            )
    ...
    if not market.empty:
        market = lower_strip_cols(market).rename(columns={"product_id": "asset"})
    ...
    pnl_summary, trades_with_pnl, stats = (
        calculate_pnl_and_metrics(trades) if not trades.empty else ({}, pd.DataFrame(), {})
    )
...
with st.expander("🔍 Data Freshness Debug", expanded=False):
    ...
    st.write(f"**Current Time (PST):** {datetime.now(pytz.utc).astimezone(pst).strftime('%Y-%m-%d %H:%M:%S %Z')}")
...
with st.sidebar:
    ...
    if st.session_state.get("auto_refresh_enabled", True):
        time_until = max(0, REFRESH_INTERVAL - int(time_since_refresh))
    ...
    # Quick data window summary (prefer trades if fresh; else market)
    date_source = "No Data"
    min_date = max_date = None
    if not trades_df.empty and "timestamp" in trades_df.columns and trades_df["timestamp"].notna().any():
        tmin, tmax = trades_df["timestamp"].min(), trades_df["timestamp"].max()
        if pd.notna(tmin) and pd.notna(tmax):
            now_utc = datetime.now(pytz.utc)
            tmax_utc = tmax.tz_localize("UTC") if tmax.tzinfo is None else tmax
            if (now_utc - tmax_utc).days <= 3:
                min_date, max_date, date_source = tmin, tmax, "Trade Data"
    if min_date is None and not market_df.empty and market_df["timestamp"].notna().any():
        min_date, max_date, date_source = market_df["timestamp"].min(), market_df["timestamp"].max(), "Market Data"

    if min_date is not None and max_date is not None:
        st.markdown(
            f"<p style='text-align:center;'><strong>{min_date.strftime('%m/%d/%y')} - {max_date.strftime('%m/%d/%y')}</strong></p>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"<p style='text-align:center;font-size:0.8em;color:grey;'>Source: {date_source}</p>",
            unsafe_allow_html=True,
        )
    else:
    ...
    if not market_df.empty and "timestamp" in market_df.columns and market_df["timestamp"].notna().any():
        ...
        if pd.notna(latest_market) and (latest_data_time is None or latest_market > latest_data_time):
            latest_data_time = latest_market
            data_source_for_time = "Latest Market Data"
    if latest_data_time:
        st.markdown(
            f"<p style='text-align:center;font-size:0.9em;color:grey;'>{data_source_for_time}: {latest_data_time.strftime('%Y-%m-%d %H:%M')}</p>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown("<p style='text-align:center;font-size:0.9em;color:grey;'>No data timestamps available</p>", unsafe_allow_html=True)

    st.markdown("---")

    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"{'✅' if not trades_df.empty else '⚠️'} **Trades:** {0 if trades_df.empty else len(trades_df)}")
    with c2:
        st.markdown(f"{'✅' if not market_df.empty else '❌'} **Market:** {0 if market_df.empty else f'{len(market_df):,}'}")
    ...
    if pnl_summary:
        ...
        for asset, pnl in sorted(pnl_summary.items(), key=lambda kv: kv[1], reverse=True):
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between'><span>{asset}</span>"
                f"<span style='color:{color};font-weight:600'>${pnl:,.2f}</span></div>",
                unsafe_allow_html=True,
            )
    else:
    ...
    if not open_positions_df.empty:
        ...
        for _, pos in open_positions_df.iterrows():
            pnlv = pos["Unrealized P&L ($)"]
            color = "#16a34a" if pnlv >= 0 else "#ef4444"
            asset_name = pos["Asset"]
            current_price = pos["Current Price"]
            price_format = ".8f" if current_price < 0.001 else ".6f" if current_price < 1 else ".2f"
            avg_entry_price = pos["Avg. Entry Price"]
            entry_price_format = ".8f" if avg_entry_price < 0.001 else ".6f" if avg_entry_price < 1 else ".2f"
            st.markdown(
                f"""<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:-10px;">
                <p style="color:{color};font-weight:bold;margin:0;">{asset_name}</p>
                <p style="color:black;font-weight:bold;margin:0;">${current_price:{price_format}}</p></div>""",
                unsafe_allow_html=True,
            )
            st.caption(f"Qty: {pos['Quantity']:.4f} | Entry: ${avg_entry_price:{entry_price_format}} | P&L: ${pnlv:.2f}")
        st.markdown("---")
    ...
    if not market_df.empty and "asset" in market_df.columns:
        ...
        for asset in all_assets:
            if asset in held_assets:
                continue
            latest = market_df[market_df["asset"] == asset]
            if not latest.empty:
                last_price = float(latest.sort_values("timestamp").iloc[-1]["close"])
                price_format = ".8f" if last_price < 0.001 else ".6f" if last_price < 1 else ".2f"
            ...
            st.markdown(
                ...
                <p style="color:grey;font-weight:bold;margin:0;">{price_str}</p></div>""",
                unsafe_allow_html=True,
            )
...
with tab1:
    ...
    if assets:
        ...
        # Dynamic “active asset”/last price readout
        if not asset_market_data.empty:
            last_price = float(asset_market_data.sort_values("timestamp").iloc[-1]["close"])
            price_format = ",.8f" if last_price < 0.001 else ",.6f" if last_price < 1 else ",.2f"
            st.metric(f"Last Price for {selected_asset}", f"${last_price:{price_format}}")

        with st.expander("🔍 Data Resolution Inspector", expanded=False):
            if not asset_market_data.empty:
                ...
                with c1:
                    ...
                    st.write(f"**Date Range:** {df_sorted['timestamp'].min():%Y-%m-%d %H:%M} → {df_sorted['timestamp'].max():%Y-%m-%d %H:%M}")
                with c2:
                ...
                with c3:
                    ...
                    for ts in recent["timestamp"]:
                        st.write(f"• {ts:%m/%d %H:%M:%S}")
        ...
        if not df.empty:
            ...
            if not vis.empty:
                st.info(
                    f"Showing {len(vis):,} candles from {start_date:%Y-%m-%d %H:%M} to {end_date:%Y-%m-%d %H:%M}"
                )
                ...
                # ML signal markers (using p_up / p_down only; confidence = |p_up - p_down|)
                    ...
                    if not prob_data.empty:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                name="ML Signals",
                                customdata=list(
                                    zip(prob_data["p_up"], prob_data["p_down"], prob_data["confidence"])
                                ),
                                hovertemplate=(
                                    "<b>ML Signal</b><br>"
                                    "Time: %{x|%Y-%m-%d %H:%M}<br>"
                ...
                # Trade markers if we have trades for this asset in the window
                if not trades_df.empty:
                    ...
                    if not asset_trades.empty:
                        buy_trades = asset_trades[
                            asset_trades["unified_action"].str.lower().isin(["buy", "open"])
                        ]
                        if not buy_trades.empty:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    name="BUY",
                                    marker=dict(
                                        symbol="triangle-up", size=14, color="#4caf50", line=dict(width=1, color="white")
                                    ),
                        ...
                        sell_trades = asset_trades[
                            asset_trades["unified_action"].str.lower().isin(["sell", "close"])
                        ]
                        if not sell_trades.empty:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    name="SELL",
                                    marker=dict(
                                        symbol="triangle-down", size=14, color="#f44336", line=dict(width=1, color="white")
                                    ),
                ...
                # Small KPIs for this asset in range
                if not trades_df.empty:
                    asset_trades = trades_df[
                        (trades_df["asset"] == selected_asset)
                        & (trades_df["timestamp"] >= start_date)
                ...
                if not asset_trades.empty:
                    ...
                    with c4:
                        if "pnl" in asset_trades.columns:
                            st.metric("Period P&L", f"${asset_trades['pnl'].sum():.6f}")
                    with c5:
                ...
                if "p_up" in vis.columns and "p_down" in vis.columns:
                    ...
                    if not prob_data.empty:
                        ...
                        with c1:
                            bull = len(prob_data[prob_data["p_up"] > prob_data["p_down"]])
                            st.metric("Bullish Signals", f"{bull} ({bull/len(prob_data)*100:.1f}%)")
                        with c2:
                            bear = len(prob_data[prob_data["p_down"] > prob_data["p_up"]])
                            st.metric("Bearish Signals", f"{bear} ({bear/len(prob_data)*100:.1f}%)")
                        with c3:
                            avg_conf = (prob_data["p_up"] - prob_data["p_down"]).abs().mean()
                            st.metric("Avg Confidence", f"{avg_conf:.3f}")
                        with c4:
                            high_conf = len(prob_data[(prob_data["p_up"] - prob_data["p_down"]).abs() > 0.1])
                            st.metric("High Confidence", f"{high_conf} (>10%)")
            else:
...
with tab2:
    if not trades_df.empty and "timestamp" in trades_df.columns and "cumulative_pnl" in trades_df.columns:
        ...
        fig_pnl.update_layout(
            title="Total Portfolio P&L", template="plotly_white", yaxis_title="P&L (USD)", xaxis_title="Date", hovermode="x unified"
        )

...
# Ensure high precision for P&L math
getcontext().prec = 30
...
# =========================
# Auto-Refresh (Option A): per-second countdown + timed cache clear
# =========================
...
def seconds_until_refresh() -> int:
    return max(0, int(st.session_state.next_refresh_at - time.time()))
...
if st.session_state.get("auto_refresh_enabled", True) and st_autorefresh is not None:
    st_autorefresh(interval=1000, key="countdown_ticker")

# When timer hits zero, clear cache, schedule next refresh, and rerun
if st.session_state.get("auto_refresh_enabled", True) and time.time() >= st.session_state.next_refresh_at:
    st.cache_data.clear()
    st.session_state.next_refresh_at = time.time() + REFRESH_INTERVAL
    st.rerun()
...
# Trade log (parquet) — double-check this ID if you see 404 errors
...
def normalize_prob_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize to columns: p_up, p_down (no p_hold here).
    """
    rename_map = {}
    p_up_variations = {"p_up", "p-up", "p up", "pup", "pup prob"}
    p_down_variations = {"p_down", "p-down", "p down", "pdown", "pdown prob"}
    for col in df.columns:
        norm = str(col).lower().replace("_", " ").replace("-", " ").strip()
...
def format_timedelta_hours_minutes(td: pd.Timedelta) -> str:
    if pd.isna(td):
        return "N/A"
...
def calculate_pnl_and_metrics(trades_df: pd.DataFrame):
    ...
    win = 0
    loss = 0
    gp = Decimal(0)  # gross profit
    gl = Decimal(0)  # gross loss
    peak = Decimal(0)
    mdd = Decimal(0)
...
def download_drive_file_bytes(url_or_id: str) -> bytes | None:
    ...
    try:
        ...
        with requests.Session() as s:
            ...
            r1 = s.get(base, params={"id": fid, "export": "download"}, timeout=60)
            if r1.status_code == 404:
                # fallback to old endpoint with confirmation cookie
                base_alt = "https://drive.google.com/uc?export=download"
                r2 = s.get(base_alt, params={"id": fid}, timeout=60)
                if "text/html" in (r2.headers.get("Content-Type") or ""):
                    m = re.search(r"confirm=([0-9A-Za-z_-]+)", r2.text)
                    if m:
                        r3 = s.get(base_alt, params={"id": fid, "confirm": m.group(1)}, timeout=60)
                        r3.raise_for_status()
                        return r3.content
                r2.raise_for_status()
                return r2.content
...
def load_data(trades_link: str, market_link: str):
    ...
    if not trades.empty:
        ...
        if "action" in trades.columns:
            trades["unified_action"] = (
                trades["action"].astype(str).str.upper().map({"OPEN": "buy", "CLOSE": "sell"}).fillna(trades["action"].astype(str).str.lower())
            )
    ...
    if not market.empty:
        ...
        # Keep floats for plotting speed
...
with st.sidebar:
    ...
    if st.session_state.get("auto_refresh_enabled", True):
        ...
        if remaining > 0:
            st.markdown(
                f"<p style='text-align:center;font-size:0.85em;color:#4CAF50;'>"
                f"⏱️ Next refresh: {m:02d}:{s:02d}</p>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<p style='text-align:center;font-size:0.85em;color:#4CAF50;'>🔄 Refreshing…</p>",
                unsafe_allow_html=True,
            )
    else:
        st.markdown(
            "<p style='text-align:center;font-size:0.85em;color:#888;'>Auto-refresh disabled</p>",
            unsafe_allow_html=True,
        )

    st.markdown("---")

    # Sidebar date range source
    date_source = "No Data"
    ...
    if latest_data_time:
        ...
        st.markdown(
            "<p style='text-align: center; font-size: 0.9em; color: grey;'>No data timestamps available</p>",
            unsafe_allow_html=True,
        )
    ...
    if not open_positions_df.empty:
        ...
        for _, pos in open_positions_df.iterrows():
            ...
            if avg_entry_price < 0.001:
                entry_price_format = ".8f"
            elif avg_entry_price < 1:
                entry_price_format = ".6f"
            else:
                entry_price_format = ".2f"
            ...
            st.caption(
                f"Qty: {pos['Quantity']:.4f} | Entry: ${avg_entry_price:{entry_price_format}} | P&L: ${pnl:.2f}"
            )
        st.markdown("---")
...
with tab1:
    ...
    if assets:
        ...
        if not df.empty:
            ...
            if not vis.empty:
                st.info(
                    f"Showing {len(vis):,} candles from {start_date.strftime('%Y-%m-%d %H:%M')} "
                ...
                # Plot ML signals: color by direction, size by confidence = |p_up - p_down|
                if "p_up" in vis.columns and "p_down" in vis.columns:
                    ...
                    if not prob_data.empty:
                        ...
                        colors = [
                            "#ff6b6b" if (pd.isna(pu) or pd.isna(pdwn) or pdwn > pu) else "#51cf66"
                            for pu, pdwn in zip(prob_data["p_up"], prob_data["p_down"])
                        ]
                        fig.add_trace(
                            go.Scatter(
                        ...
                        if not buy_trades.empty:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    hovertemplate="<b>BUY</b> @ $%{customdata[0]:.8f}<br>%{x|%H:%M:%S}<br>"
                        ...
                        if not sell_trades.empty:
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    hovertemplate="<b>SELL</b> @ $%{customdata[0]:.8f}<br>%{x|%H:%M:%S}<br>"
                                    "Reason: %{customdata[1]}<extra></extra>",
                                )
                            )
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(
                        title="Price (USD)",
                        tickformat=(
                            ".8f" if vis["close"].iloc[-1] < 0.001 else ".6f" if vis["close"].iloc[-1] < 1 else ".4f"
                        ),
                        showgrid=True,
                        gridcolor="rgba(128,128,128,0.1)",
                ...
                # Period summary for this asset/range
                asset_trades = (
                    trades_df[
                        (trades_df["asset"] == selected_asset)
                        & (trades_df["timestamp"] >= start_date)
                ...
                # ML signal analysis block
                if "p_up" in vis.columns and "p_down" in vis.columns:
                    st.markdown("---")
                    st.markdown("### 🤖 ML Signal Analysis")
                    prob_data = vis.dropna(subset=["p_up", "p_down"])
                    if not prob_data.empty:
                        bullish_signals = int((prob_data["p_up"] > prob_data["p_down"]).sum())
                        bearish_signals = int((prob_data["p_down"] > prob_data["p_up"]).sum())
                        total_signals = len(prob_data)
                        avg_confidence = (prob_data["p_up"] - prob_data["p_down"]).abs().mean()
                        high_conf = int(((prob_data["p_up"] - prob_data["p_down"]).abs() > 0.10).sum())

                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            pct = (bullish_signals / total_signals * 100) if total_signals else 0
                            st.metric("Bullish Signals", f"{bullish_signals} ({pct:.1f}%)")
                        with col2:
                            pct = (bearish_signals / total_signals * 100) if total_signals else 0
                            st.metric("Bearish Signals", f"{bearish_signals} ({pct:.1f}%)")
                        with col3:
